---
id: OAA9Hd
title: 多标签页并发请求导致 Token 刷新失败？只有 15行代码就能解决 !
createdAt: "2026-06-02 15:50:01"
updated: "2026-06-02 15:50:01"
tags: []
tag_ids: []
categories: []
published: true
...
![多标签页并发请求导致 Token 刷新失败？只有 15行代码就能解决 !](https://p3-xtjj-sign.byteimg.com/tos-cn-i-73owjymdk6/1f76d9c68b7640d990edc8f75bd3e203~tplv-73owjymdk6-jj-mark-v1:0:0:0:0:5o6Y6YeR5oqA5pyv56S-5Yy6IEAgRXJwYW5PbWVy:q75.awebp?rk3s=f64ab15b&x-expires=1780911756&x-signature=tcfSo8bLySqKihrhEcorzRGah3g%3D)
...
原因极其典型：用户在浏览器里开着 5 个我们的后台标签页，半小时后，Token 过期了。 5 个标签页 **在同一瞬间** 检测到了过期，同时向后端发起 `refreshToken` 请求。

而我们的后端为了安全，设计了 **单次刷新令牌即失效（One-Time Use Token）** 的安全机制。 当这 5 个并发请求几乎同时到达服务器时：

![ChatGPT Image 2026年6月1日 16_10_10.png](https://p3-xtjj-sign.byteimg.com/tos-cn-i-73owjymdk6/5fb4a2a391154c61ba82cfb2ec0f067e~tplv-73owjymdk6-jj-mark-v1:0:0:0:0:5o6Y6YeR5oqA5pyv56S-5Yy6IEAgRXJwYW5PbWVy:q75.awebp?rk3s=f64ab15b&x-expires=1780911756&x-signature=POR0busfywaQYtiFufcRAJjtCiA%3D)
...
其实，现代浏览器早就为我们内置了一款低调、极其强大、却被 90% 前端忽略的冷门大杀器—— **[`Web Locks API`](https://link.juejin.cn/?target=https%3A%2F%2Fdeveloper.mozilla.org%2Fzh-CN%2Fdocs%2FWeb%2FAPI%2FWeb_Locks_API "https://developer.mozilla.org/zh-CN/docs/Web/API/Web_Locks_API") （Web 锁 API）** 。
...
`navigator.locks` 是 `W3C` 正式通过的标准 `API` （早在 2022 年就已被所有主流浏览器原生支持）。 它允许同源（Same-Origin）下的多个浏览器上下文（无论是多个 `Tab` 标签页，还是多个 `Web Worker` 线程），去异步申请一个 **互斥的共享资源锁** 。
...
![ChatGPT Image 2026年6月1日 16_30_32.png](https://p3-xtjj-sign.byteimg.com/tos-cn-i-73owjymdk6/aeab4d9cd86c4bfa967d1b079269e3c0~tplv-73owjymdk6-jj-mark-v1:0:0:0:0:5o6Y6YeR5oqA5pyv56S-5Yy6IEAgRXJwYW5PbWVy:q75.awebp?rk3s=f64ab15b&x-expires=1780911756&x-signature=2W0K3IRqKcrVukNDymFJLpumHpU%3D)

核心伪源码👇:

```typescript
体验AI代码助手 代码解读复制代码// 纯原生 Web Locks 优雅解决多标签页并发刷新
...
![ChatGPT Image 2026年6月1日 17_15_10.png](https://p3-xtjj-sign.byteimg.com/tos-cn-i-73owjymdk6/b32d55e96be6460daee477131370c5a9~tplv-73owjymdk6-jj-mark-v1:0:0:0:0:5o6Y6YeR5oqA5pyv56S-5Yy6IEAgRXJwYW5PbWVy:q75.awebp?rk3s=f64ab15b&x-expires=1780911756&x-signature=AWZW09pQWZHj4o4izfgNMI%2FhPwQ%3D)

```typescript
体验AI代码助手 代码解读复制代码// 带超时控制的 Web 锁
